...
for indx,i in enumerate(files):
    ...
    df = pd.read_excel(file_path, sheet_name='Envelope TOT')
    if indx == 0:
        bins = pd.read_excel(file_path, sheet_name='Bins of TOT')
    keys = list(df.keys())

    for e in keys:
        if 'EF-P1' in e: 
            ns[mouse[indx]+'_'+e] = df[e]
bins = (np.asarray(bins))[:,0]
data = {}

protocols = [x.split("-")[-1] for x in ns.keys()]
protocols = set([x.split("_")[0] for x in protocols])
for i in protocols:
    stim,nostim = [],[]
    for (key,values) in ns.items():
        if i in key and '_Stim' in key:
            norm_vs = (values/np.max(values))
            stim.append(norm_vs)
        if i in key and '_NoStim' in key:
...
for ind,i in enumerate(protocols):
    ...
    fig.legend()
    fig.suptitle('Median envelope of all animals per protocol')
    # fig.suptitle(f'{topo}_Envelope per protocol')
fig.savefig(savedir+f'/All_animals_NoStim vs Stim.pdf')
plt.close()
